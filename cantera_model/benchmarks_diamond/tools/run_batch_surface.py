"""
Generic isothermal batch reactor with a surface.

This is intentionally simple so it can be used as a reduction benchmark driver.

Notes:
- Uses IdealGasConstPressureReactor to avoid pressure drift.
- energy='off' for isothermal (common for kinetics benchmarking).
"""
...
class BatchSurfaceConfig:
    ...
    T: float
    P_pa: float
    X: str  # Cantera composition string
    t_end: float = 1.0
    n_steps: int = 200
    area: float = 1.0  # m^2-equivalent in Cantera units (actually cm^2 if YAML uses cm)
...
def run_batch_surface(cfg: BatchSurfaceConfig, save_rates: bool = False) -> None:
    ...
    # Load interface and adjacent gas
    ...
    # Optional: full reaction rates-of-progress can get large
    gas_rop = None
    surf_rop = None
    if save_rates:
