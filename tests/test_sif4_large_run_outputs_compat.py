...
def test_sif4_large_run_outputs_compat() -> None:
    if cantera is None:
        pytest.skip("cantera is required")

    run_id = "test_sif4_benchmark_large"
    proc = subprocess.run(
        [
            sys.executable,
            "run_cantera_eval.py",
            "--config",
            "configs/sif4_benchmark_sin3n4_large_quick.yaml",
            "--run-id",
            run_id,
        ],
        capture_output=True,
    ...
    assert summary["surface_phase"] == "SI3N4"
