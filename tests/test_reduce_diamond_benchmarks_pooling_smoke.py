...
def test_reduce_diamond_benchmarks_pooling_smoke(tmp_path: Path) -> None:
    if not cantera_available():
        pytest.skip("cantera is required")

    trace_path = build_diamond_trace(tmp_path, run_id="diamond_trace_pooling")
    summary_path = run_reduce_mode(
        tmp_path=tmp_path,
        config_path="configs/reduce_diamond_benchmarks_pooling.yaml",
        trace_path=trace_path,
        run_id="reduce_diamond_benchmarks_pooling_test",
    )
