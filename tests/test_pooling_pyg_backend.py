...
def test_pooling_pyg_backend_seeded_inference_is_deterministic() -> None:
    try:
        import torch  # noqa: F401
        from torch_geometric.nn import GCNConv  # noqa: F401
    except Exception:
    ...
    feat = np.random.default_rng(11).normal(size=(n_species, 10))
    cfg = {
        "model": {"backend": "pyg", "n_clusters": 3, "hidden_dim": 12, "seed": 7, "dropout": 0.4},
        "train": {"temperature": 0.7},
    }

    model_a = build_pooling_model(input_dim=feat.shape[1], cfg=cfg)
    model_b = build_pooling_model(input_dim=feat.shape[1], cfg=cfg)
    S_a1 = infer_assignment(model_a, graph, feat, {"train": {"temperature": 0.7}})
    S_a2 = infer_assignment(model_a, graph, feat, {"train": {"temperature": 0.7}})
    S_b = infer_assignment(model_b, graph, feat, {"train": {"temperature": 0.7}})

    assert np.allclose(S_a1, S_a2, atol=1.0e-12)
    assert np.allclose(S_a1, S_b, atol=1.0e-12)
