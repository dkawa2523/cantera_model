...
def test_diamond_benchmarks_diamond_run_outputs() -> None:
    if cantera is None:
        pytest.skip("cantera is required")

    run_id = "test_diamond_benchmarks_diamond"
    proc = subprocess.run(
        [
            sys.executable,
            "run_cantera_eval.py",
            "--config",
            "configs/diamond_benchmarks_diamond_quick.yaml",
            "--run-id",
            run_id,
        ],
        capture_output=True,
    ...
    assert summary["surface_phase"] == "diamond_100"
