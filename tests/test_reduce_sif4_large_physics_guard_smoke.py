...
def test_reduce_sif4_large_physics_guard_smoke(tmp_path: Path) -> None:
    if not cantera_available():
        pytest.skip("cantera is required")

    trace_path = build_sif4_large_trace(tmp_path, run_id="sif4_trace_physics_guard")
    summary_path = run_reduce_mode(
        tmp_path=tmp_path,
        config_path="configs/reduce_sif4_benchmark_sin3n4_large_pooling.yaml",
        trace_path=trace_path,
        run_id="reduce_sif4_large_pooling_phys_test",
    )
    out = json.loads(summary_path.read_text())
    sel = dict(out["selected_metrics"])
    assert int(sel.get("floor_min_reactions", 0)) >= 10
