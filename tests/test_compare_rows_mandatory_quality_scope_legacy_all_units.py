...
def test_mandatory_quality_scope_all_units_keeps_legacy_behavior() -> None:
    baseline = [
        ...
        eval_policy={
            "error_aggregation": {
                **common_error,
                ...
                "mandatory_tail_scope": "all_units",
            }
        },
    )
    ...
    assert legacy_all["mandatory_quality_scope_effective"] == "all_units"
    assert legacy_all["mandatory_tail_scope_effective"] == "all_units"
    assert float(legacy_all["pass_rate_mandatory_case"]) == float(
        legacy_all["pass_rate_mandatory_case_all_units"]
    )
    assert legacy_all["mandatory_quality_passed"] is False
    assert legacy_all["error_fail_reason_primary"] == "mandatory_case_rate"
    assert float(valid_only["pass_rate_mandatory_case"]) > float(
        legacy_all["pass_rate_mandatory_case"]
    )
