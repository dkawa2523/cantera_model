...
def test_reduce_ac_large_learnckpp_smoke(tmp_path: Path) -> None:
    if not cantera_available():
        pytest.skip("cantera is required")

    trace_path = build_ac_large_trace(tmp_path, run_id="ac_trace_learnckpp")
    summary_path = run_reduce_mode(
        tmp_path=tmp_path,
        config_path="configs/reduce_ac_benchmark_large_learnckpp.yaml",
        trace_path=trace_path,
        run_id="reduce_ac_large_learnckpp_test",
    )
