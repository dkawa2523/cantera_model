...
def test_pooling_coverage_repair_increases_cluster_diversity() -> None:
    meta = [
    ...
    assert int(out["train_metrics"]["n_clusters"]) >= 3
    assert float(out["train_metrics"]["coverage_proxy"]) >= 0.55
