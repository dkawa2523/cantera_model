...
def test_reduce_sif4_large_baseline_smoke(tmp_path: Path) -> None:
    if not cantera_available():
        pytest.skip("cantera is required")

    trace_path = build_sif4_large_trace(tmp_path, run_id="sif4_trace_baseline")
    summary_path = run_reduce_mode(
        tmp_path=tmp_path,
        config_path="configs/reduce_sif4_benchmark_sin3n4_large_baseline.yaml",
        trace_path=trace_path,
        run_id="reduce_sif4_large_baseline_test",
    )
    ...
    assert out["selected_metrics"]["species_before"] >= 50
    assert out["selected_metrics"]["reactions_before"] >= 100
