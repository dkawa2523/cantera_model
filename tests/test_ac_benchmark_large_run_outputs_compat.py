...
def test_ac_benchmark_large_run_outputs_compat() -> None:
    if cantera is None:
        pytest.skip("cantera is required")

    run_id = "test_ac_benchmark_large"
    proc = subprocess.run(
        [
            sys.executable,
            "run_cantera_eval.py",
            "--config",
            "configs/ac_benchmark_large_quick.yaml",
            "--run-id",
            run_id,
        ],
        capture_output=True,
    ...
    assert summary["surface_phase"] == "ac_surf"
